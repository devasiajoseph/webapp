...
import (
	...
	"github.com/devasiajoseph/webapp/settings"
	"github.com/devasiajoseph/webapp/sms"
	...
	"github.com/devasiajoseph/webapp/mail"
...
func (ua *UserAccount) sendForgotPasswordOTP() (UserKeys, error) {

	uk, err := ua.GenerateOTP(passwordResetKey)
	if err != nil {
		log.Println(err)

	}

	tempSettings, err := settings.GetSettings(settings.OTPForgotPasswordTemplateID)
	if err != nil {
...
func (ua *UserAccount) sendRegistrationOTP() (UserKeys, error) {

	uk, err := ua.GenerateOTP(registrationKey)
	if err != nil {
		log.Println(err)
...
func (uk *UserKeys) verifyOTP(otp string) bool {
	err := uk.Data()
	if err != nil {
		log.Println("Error verifying otp")
		log.Println(err)
		return false
	}
	if uk.OTP == otp {
		err = uk.UpdateKey(crypt.UID())
		if err != nil {
			return false
		}
		return true
